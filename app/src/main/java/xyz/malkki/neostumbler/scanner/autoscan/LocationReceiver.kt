...
class LocationReceiver : BroadcastReceiver() {
    ...
    private fun tryStartAutoscan(context: Context, currentLocation: Location) {
        Timber.i("Removing geofences because new location was received")

        val geofencingClient = LocationServices.getGeofencingClient(context)

        //Can't use coroutine API .await() here because it causes geofence removal request to hang for some reason
                ...
                if (canStartScanningHere) {
                    ...
                    val geofencingRequest = GeofencingRequest.Builder()
                        .addGeofence(Geofence.Builder()
                            ...
                            .setCircularRegion(currentLocation.latitude, currentLocation.longitude, REPORT_RADIUS.toFloat() * 1.2f) //Use slightly larger radius for geofence to make it more likely that scanning is possible when user exits geofence
