...
fun BatteryOptimizationsDialog(onBatteryOptimizationsDisabled: (Boolean) -> Unit) {
    ...
    val batteryOptimizationActivityLauncher =
        rememberLauncherForActivityResult(
            contract = ActivityResultContracts.StartActivityForResult(),
            onResult = { _ ->
                coroutineScope.launch { oneTimeActionHelper.markActionShown(ACTION_NAME) }
                ...
                if (!batteryOptimizationsDisabled) {
                    context.showToast(
                        ContextCompat.getString(
                            context,
                            R.string.battery_optimizations_not_disabled_warning,
                        )
                    )
                }
            },
    ...
    AlertDialog(
        ...
        confirmButton = {
            TextButton(
                onClick = {
                    batteryOptimizationActivityLauncher.launch(
