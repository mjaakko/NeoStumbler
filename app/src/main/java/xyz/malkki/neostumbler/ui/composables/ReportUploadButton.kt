...
fun ReportUploadButton(
    ...
    val uploadEnqueued by
        ichnaeaReportUploadStarter.reportUploadEnqueued.collectAsStateWithLifecycle(
            initialValue = true
        )
    ...
    FilledIconButton(
        modifier = modifier,
        enabled = reportsNotUploaded.value > 0 && !uploadEnqueued && !enqueuing.value,
        onClick = {
            coroutineScope.launch {
                enqueuing.value = true

                enqueuedUploadWork.value =
                    ichnaeaReportUploadStarter.startUploadNotUploadedReports()
